...
Direct Line 3.0 スキーマには、[Bot Framework アクティビティ スキーマ](https://aka.ms/botSpecs-activitySchema)によって定義されるすべてのオブジェクトに加え、`ActivitySet` オブジェクトと `Conversation` オブジェクトが含まれています。
