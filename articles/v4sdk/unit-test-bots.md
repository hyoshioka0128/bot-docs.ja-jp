...
`BookingDialog` や `DateResolverDialog` のような一部のダイアログは、呼び出し元のダイアログに値を返します。 `DialogTestClient` オブジェクトは、ダイアログで返された結果を分析およびアサートするために使用できる `dialogTurnResult` プロパティを公開します。
