...
[LUISGen ツール](https://aka.ms/botbuilder-tools-luisgen)を使用して、ボットのコードで LUIS からエンティティを簡単に抽出できるクラスを生成できます。
...
```
また、`Entities` プロパティもあります。 ユーザーのメッセージにはエンティティが複数回出現する可能性があるため、`_Entities` クラスではエンティティの種類ごとに配列を定義します。 
```cs
