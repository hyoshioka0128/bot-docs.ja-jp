...
Bot Framework SDK には、インメモリ ストレージなどのストレージ レイヤーの実装 (ローカルでのテスト用)、および Azure Storage または CosmosDB (クラウドでのテストとデプロイ用) が用意されています。
...
これらのバケットはすべて "*ボット状態*" クラスのサブクラスであり、これを基に他の種類のバケットを定義できます。
