...
`dialogs/mainDialog/index.js` ファイルで、`userState` を 2 番目の引数として受け入れるようにコンストラクターを更新します。 次に、`topicStates` プロパティを `conversationState` から作成し、`userProfile` プロパティを `userState` から作成します。
